...
## Wizualizacja wybranego podgrafu (dopiero po selekcji)

```text
Wybierz najważniejszy podgraf i przedstaw go jako diagram Mermaid `flowchart` bezpośrednio w raporcie Markdown.

Nie wizualizuj całego analizowanego obszaru. Ogranicz graf do wskazanych modułów, ich bezpośrednich zależności oraz wybranych aktywnych konsumentów tak, aby odpowiadał na jedno pytanie z analizy. Rozróżnij zwykłe zależności pakietowe od odwrotnych zależności wykrytych na poziomie kodu, używając innych stylów krawędzi i dodając legendę.
```
